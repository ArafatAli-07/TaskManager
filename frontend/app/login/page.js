...
const Login = () => {                 
    ...
    const LoginHandler = async(e)=>{
        ...
        // console.log(input);
        try {
            const res = await axios.post(`https://taskmanager-et0d.onrender.com/api/v1/user/login`, input,{
                headers:{
    ...
    <div className='flex justify-center items-center bg-black w-screen h-screen text-white'>
      <div className='h-lg w-lg border-2 border-slate-400 py-7 rounded-lg mx-3 md:mx-0'>
        ...
        <div className='flex flex-col justify-center items-center'>
            <h1 className='font-serif text-2xl font-bold mt-2'>Login</h1>
            <span className='italic font-serif text-slate-400'>Manage your task at one place.</span>
        </div>
        <form onSubmit={LoginHandler} className='flex flex-col mx-5 '>
            ...
            <button type="submit" className='bg-blue-500 flex item-center justify-center text-white mt-4 p-1 rounded-sm'>Login</button>

            <div className='text-[13px] flex justify-end items-center mr-2 mt-1'><span className='flex'>Already have an account?<Link href='/signup' className='text-blue-500 underline underline-offset-2 font-semibold'>&nbsp;SignUp</Link></span></div>
